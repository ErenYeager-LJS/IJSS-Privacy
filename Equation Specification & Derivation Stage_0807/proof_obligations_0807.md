...
| PO-13 | Bootstrap actuator and funnel feasibility | Verify, before composite gain closure, that there exists a joint choice of funnel endpoints, deadlines, preliminary gains, residual allocation, and compact bootstrap set `K_0` satisfying ES-38, ES-67, ES-68, ES-95 and `u_i in U_i` for all states in `K_0`. This is a design-domain feasibility check, not a consequence of Theorem 1 or ES-102. | ES-12, ES-22--ES-31, ES-62, ES-67, ES-68, ES-95 | Assumption 1; Assumption 2; PO-02A; PO-03; PO-16A | Explicit feasibility test and design inequalities linking deadlines, input limits, residual allocation, and `K_0`; no use of ES-51, ES-102, or PO-07. | PO-07; PO-11; PO-16B; Theorems 1--2 | OPEN | Equation revision if no feasible retained-controller design exists on any admissible bootstrap set; Blueprint reopen only if the architecture cannot be made feasible. |
...
| PO-02A | PROVED SUBJECT TO PO-03 | 2026-08-08 (conditional) | `derivation_stage_1_0807.md`, PO-02A scope | YES |
...
The Stage-2.5 repair has been incorporated into the authoritative ledger. Its former aggregate `PO-16` dependency is split here into local well-posedness `PO-16A` and forward continuation `PO-16B`. The former aggregate `PO-02` is split into local finite residual boundedness `PO-02A` and the later decaying envelope `PO-02B`. `PO-08` and `PO-09` depend only on `PO-16A` and PO-02A at the bootstrap stage; `PO-10` remains conditionally closed on `PO-03`. No status is upgraded to `PROVED`, and the Equation Freeze gate remains conditional.
