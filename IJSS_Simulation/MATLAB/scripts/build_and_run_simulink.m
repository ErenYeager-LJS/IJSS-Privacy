...
set_param(mdl,'Solver','ode45','RelTol',nstr(P.rtol),'AbsTol',nstr(P.atol), ...
    ...
    'OutputTimes',sprintf('[0:0.005:%.15g %.15g]',P.stoptime,P.stoptime), ...
...
add_scopes([mdl '/Scopes'],[1160 315 1390 535]);
...
for k=1:8
    add_block('simulink/Signal Routing/Mux',[mdl '/' families{k}], ...
        'Inputs','3','Position',[205 45+50*(k-1) 210 75+50*(k-1)]);
    for i=1:3
...
add_line(mdl,'Privacy_Mechanism/5','Electrical_Model/5','autorouting','on');
add_line(mdl,'Privacy_Mechanism/6','Electrical_Model/6','autorouting','on');
...
add_line(mdl,'Electrical_Model/7','Secondary_Controller/4','autorouting','on');
add_line(mdl,'Electrical_Model/8','Secondary_Controller/5','autorouting','on');
...
add_line(mdl,'Electrical_Model/5','Power_Monitor_Mux/1','autorouting','on');
add_line(mdl,'Electrical_Model/6','Power_Monitor_Mux/2','autorouting','on');
...
add_line(mdl,'Secondary_Controller/1','Privacy_Mechanism/1','autorouting','on');
add_line(mdl,'Secondary_Controller/2','Privacy_Mechanism/2','autorouting','on');
...
for k=1:4
    add_block('simulink/Signal Routing/Demux',[mdl sprintf('/d%s_split',families{k})], ...
        'Outputs','3','Position',[1115 40+50*(k-1) 1120 70+50*(k-1)]);
    add_line(mdl,sprintf('Physical_Derivatives/%d',k),sprintf('d%s_split/1',families{k}),'autorouting','on');
    for i=1:3, add_line(mdl,sprintf('d%s_split/%d',families{k},i),sprintf('DG%d/%d',i,k),'autorouting','on'); end
end
for k=5:8
    add_block('simulink/Signal Routing/Demux',[mdl sprintf('/d%s_split',families{k})], ...
        'Outputs','3','Position',[1115 265+50*(k-5) 1120 295+50*(k-5)]);
...
vnames={'V1','V2','V3','Vdot1','Vdot2','Vdot3','omega1','omega2','omega3', ...
    'delta1','delta2','delta3','pV1','pV2','pV3','qV1','qV2','qV3', ...
    'pW1','pW2','pW3','qW1','qW2','qW3'};
...
for k=1:8
    ...
    add_block('simulink/Continuous/Integrator',[path '/' outs{k}], ...
        'InitialCondition',nstr(P.x0(i+(k-1)*3)),'Position',[95 17+38*(k-1) 125 37+38*(k-1)]);
...
% Power-flow network uses two explicit line subsystems for the path graph.
for k=[1 4]
    add_block('simulink/Signal Routing/Demux',[path sprintf('/split_%d',k)],'Outputs','3','Position',[85 20+55*(k-1) 90 55+55*(k-1)]);
    add_line(path,[inNames{k} '/1'],sprintf('split_%d/1',k));
end
add_edge([path '/Line_12'],P.B(1,2),[135 20 270 105]);
add_edge([path '/Line_23'],P.B(2,3),[135 125 270 210]);
edgeMap=[1 2;2 3];
for e=1:2
    for j=1:2
        node=edgeMap(e,j);
...
% Assemble P and Q at the three nodes.
for i=1:3
    add_block('simulink/Math Operations/Sum',[path sprintf('/Psum%d',i)],'Inputs',p_signs(i),'Position',[315 20+38*(i-1) 345 45+38*(i-1)]);
    add_block('simulink/Sources/Constant',[path sprintf('/Pload%d',i)],'Value',nstr(P.Pload(i)),'Position',[275 22+38*(i-1) 300 42+38*(i-1)]);
    add_line(path,sprintf('Pload%d/1',i),sprintf('Psum%d/1',i));
    add_block('simulink/Math Operations/Sum',[path sprintf('/Qsum%d',i)],'Inputs',q_signs(i),'Position',[315 155+38*(i-1) 345 180+38*(i-1)]);
    add_block('simulink/Sources/Constant',[path sprintf('/Qload%d',i)],'Value',nstr(P.Qload(i)),'Position',[275 157+38*(i-1) 300 177+38*(i-1)]);
    add_line(path,sprintf('Qload%d/1',i),sprintf('Qsum%d/1',i));
end
add_line(path,'Line_12/1','Psum1/2'); add_line(path,'Line_12/1','Psum2/2');
add_line(path,'Line_23/1','Psum2/3'); add_line(path,'Line_23/1','Psum3/2');
add_line(path,'Line_12/2','Qsum1/2'); add_line(path,'Line_12/3','Qsum2/2');
add_line(path,'Line_23/2','Qsum2/3'); add_line(path,'Line_23/3','Qsum3/2');
add_block('simulink/Signal Routing/Mux',[path '/P'],'Inputs','3','Position',[380 25 385 115]);
add_block('simulink/Signal Routing/Mux',[path '/Q'],'Inputs','3','Position',[380 160 385 250]);
for i=1:3, add_line(path,sprintf('Psum%d/1',i),sprintf('P/%d',i)); add_line(path,sprintf('Qsum%d/1',i),sprintf('Q/%d',i)); end
% Vector droop drift maps.
bias_gain_sum(path,'Perr','P/1',-P.Pd(:),diag(P.kP(:)),[430 20]);
bias_gain_sum(path,'Werr','omega/1',-P.Wref,eye(3),[430 70]);
...
bias_gain_sum(path,'Verr','V/1',-P.Vref,-eye(3),[430 190]);
...
bias_gain_sum(path,'e0V','V/1',-P.Vref,eye(3),[105 25]);
bias_gain_sum(path,'e0omega','omega/1',-P.Wref,eye(3),[105 85]);
...
add_schedule([path '/Schedule'],P.rho0V(:),P.rhoinfV(:),P.TV,[70 15 220 95]);
divide(path,'sigma','e0/1','Schedule/1',[265 25]);
trigblock(path,'zeta','sigma/1','atanh',[330 25]);
product(path,'sigma2',{'sigma/1','sigma/1'},[330 70]); constant(path,'ones',ones(3,1),[330 110]);
...
add_schedule([path '/Schedule'],P.rho0W(:),P.rhoinfW(:),P.TW,[70 15 220 95]);
...
function add_schedule(path,r0,ri,T,pos)
add_block('simulink/Ports & Subsystems/Subsystem',path,'Position',pos); clear_sub(path);
add_block('simulink/Sources/Clock',[path '/Clock'],'Position',[15 20 40 40]); gainblock(path,'s','Clock/1',1/T,[70 20]);
...
constant(path,'Tconst',T,[435 205]); add_block('simulink/Logic and Bit Operations/Relational Operator',[path '/beforeT'],'Operator','<=','Position',[500 205 535 235]); add_line(path,'Clock/1','beforeT/1'); add_line(path,'Tconst/1','beforeT/2');
for item={'h','dh','ddh'}
...
constant(path,['ones' sfx],ones(3,1),[330 y+75]); add_block('simulink/Math Operations/MinMax',[path '/g' sfx],'Function','min','Inputs','2','Position',[400 y+35 435 y+65]); add_line(path,['ratio' sfx '/1'],['g' sfx '/1']); add_line(path,['ones' sfx '/1'],['g' sfx '/2']);
...
scope_pair(path,'Voltage_Scope','V',95,20); scope_pair(path,'Frequency_Scope','omega',95,70);
add_block('simulink/Signal Routing/Mux',[path '/Control_Mux'],'Inputs','2','Position',[95 120 100 165]); add_line(path,'uV/1','Control_Mux/1'); add_line(path,'uomega/1','Control_Mux/2');
add_block('simulink/Sinks/Scope',[path '/Control_Input_Scope'],'Position',[150 125 205 160]); add_line(path,'Control_Mux/1','Control_Input_Scope/1');
add_block('simulink/Signal Routing/Mux',[path '/Public_Mux'],'Inputs','2','Position',[95 185 100 230]); add_line(path,'public_pV/1','Public_Mux/1'); add_line(path,'public_pomega/1','Public_Mux/2');
add_block('simulink/Sinks/Scope',[path '/Public_Message_Scope'],'Position',[150 190 205 225]); add_line(path,'Public_Mux/1','Public_Message_Scope/1');
add_block('simulink/Signal Routing/Mux',[path '/Private_Mux'],'Inputs','2','Position',[95 250 100 295]); add_line(path,'private_qV/1','Private_Mux/1'); add_line(path,'private_qomega/1','Private_Mux/2');
add_block('simulink/Sinks/Scope',[path '/Internal_Private_Scope'],'Position',[150 255 205 290]); add_line(path,'Private_Mux/1','Internal_Private_Scope/1');
end

function scope_pair(path,name,src,x,y)
add_block('simulink/Sinks/Scope',[path '/' name],'Position',[x y x+55 y+30]); add_line(path,[src '/1'],[name '/1']);
end
...
function constant(path,name,value,pos), add_block('simulink/Sources/Constant',[path '/' name],'Value',nstr(value),'Position',[pos(1) pos(2) pos(1)+35 pos(2)+20]); end
function sumblock(path,name,signs,pos), add_block('simulink/Math Operations/Sum',[path '/' name],'Inputs',signs,'Position',[pos(1) pos(2) pos(1)+30 pos(2)+30]); end
function gainblock(path,name,src,gain,pos), add_block('simulink/Math Operations/Gain',[path '/' name],'Gain',nstr(gain),'Multiplication','Matrix(K*u)','Position',[pos(1) pos(2) pos(1)+55 pos(2)+25]); add_line(path,src,[name '/1']); end
function product(path,name,sources,pos), add_block('simulink/Math Operations/Product',[path '/' name],'Inputs',repmat('*',1,numel(sources)),'Multiplication','Element-wise(.*)','Position',[pos(1) pos(2) pos(1)+35 pos(2)+25]); for k=1:numel(sources), add_line(path,sources{k},sprintf('%s/%d',name,k)); end, end
function divide(path,name,num,den,pos), add_block('simulink/Math Operations/Product',[path '/' name],'Inputs','*/','Multiplication','Element-wise(.*)','Position',[pos(1) pos(2) pos(1)+35 pos(2)+25]); add_line(path,num,[name '/1']); add_line(path,den,[name '/2']); end
function trigblock(path,name,src,op,pos), add_block('simulink/Math Operations/Trigonometric Function',[path '/' name],'Operator',op,'Position',[pos(1) pos(2) pos(1)+45 pos(2)+25]); add_line(path,src,[name '/1']); end
