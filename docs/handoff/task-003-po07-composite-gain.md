...
The non-redundant gain certificate is `Q_cl = Q_0 - H^T W_D H >> 0`. Under this certificate, ES-102 is derived explicitly with `a_cl=lambda_cl/(2M_V)`, an explicit `d_R^*`, and finite local `d_priv(t)`. The result is a local Lyapunov comparison inequality on `K_0`, not global stability.
...
- Cancellation audit: voltage, frequency, privacy, graph, residual, uncertainty, and Young terms each occur once; no cross-channel term exists.
