...
| PO-07 | Composite gain conditions | Convert ES-94, ES-98, and ES-101 plus PO-06 into explicit sufficient inequalities guaranteeing `a_cl>0` in ES-102 on `K_0`; state all Young constants, graph/gain restrictions, and compatibility with the pre-checked actuator/funnel design domain. | ES-90--ES-102 | Assumptions 1--2; PO-02; PO-03; PO-06; PO-08--PO-10; PO-13 | A complete set of verifiable gain inequalities and formulas for `a_cl,d_R,d_priv`. | PO-11; Theorem 1; Theorem 2 | OPEN | Equation revision if no feasible positive-gain region exists; Blueprint reopen only if a missing stabilizing mechanism is necessary. |
| PO-08 | Voltage Lyapunov chain on bootstrap set | Close ES-83--ES-95, including cancellation, graph/residual/uncertainty bounds, and compatibility of the voltage terms with the PPC transformation on `K_0`. | ES-26--ES-29, ES-36, ES-62--ES-67, ES-83--ES-95 | Assumption 1; Lemma 1/PO-02; PO-06; PO-16A | Formal voltage derivative inequality with positive coefficients and specified constants on the local domain. | PO-07; PO-11; Theorem 1 | PROVED SUBJECT TO PO-16A | Pointwise weighted chain is closed; full trajectory continuation remains in PO-11 and PO-16B. |
| PO-09 | Frequency Lyapunov chain on bootstrap set | Close ES-85--ES-98, including transformation-gain bounds and graph/residual/uncertainty terms on `K_0`. | ES-30--ES-32, ES-37, ES-68--ES-70, ES-85--ES-98 | Assumption 1; Lemma 1/PO-02; PO-06; PO-16A | Formal frequency derivative inequality with positive coefficients and specified constants on the local domain. | PO-07; PO-11; Theorem 1 | PROVED SUBJECT TO PO-16A | Pointwise chain is closed; full trajectory continuation remains in PO-11 and PO-16B. |
| PO-10 | Privacy Lyapunov chain | Close ES-87--ES-101 by applying PO-01 and the local command-rate estimate PO-03; give explicit positive `a_z,a_r` and finite `d_c`. | ES-43--ES-51, ES-87--ES-101 | Assumption 2; PO-01; PO-03 | Channel-combined privacy inequality, with conditions on `eps_r1,eps_r2` and private-weight margins. | PO-07; Theorem 1 | PROVED SUBJECT TO PO-03 | Metric normalization is repaired; explicit private-weight feasibility remains. It does not use ES-51. |
...
| PO-12 | Practical prescribed-time recovery | Combine the forward continuation result, the quintic schedule, and ES-35 to prove post-deadline entry into the declared nonzero tolerances and state precisely the residual/uncertainty ultimate bound. | ES-22--ES-40, ES-51, ES-102--ES-103 | Assumptions 1--2; PO-02; PO-07; PO-11; PO-16B | Theorem-ready deadline statement for `T_V,T_omega`, with no exact-zero claim. | Theorem 2; Theorem 3; Theorem 4 | OPEN | Equation revision if the claimed tolerance is smaller than the proven post-deadline bound; no Blueprint reopen for a parameter-only change. |
| PO-13 | Bootstrap actuator and funnel feasibility | Verify, before composite gain closure, that there exists a joint choice of funnel endpoints, deadlines, preliminary gains, residual allocation, and compact bootstrap set `K_0` satisfying ES-38, ES-67, ES-68, ES-95 and `u_i in U_i` for all states in `K_0`. This is a design-domain feasibility check, not a consequence of Theorem 1 or ES-102. | ES-12, ES-22--ES-31, ES-51, ES-62, ES-67, ES-68, ES-95 | Assumption 1; Assumption 2; PO-02; PO-03; PO-16A | Explicit feasibility test and design inequalities linking deadlines, input limits, residual allocation, and `K_0`; no use of ES-102 or PO-07. | PO-07; PO-11; PO-16B; Theorems 1--2 | OPEN | Equation revision if no feasible retained-controller design exists on any admissible bootstrap set; Blueprint reopen only if the architecture cannot be made feasible. |
...
| PO-16A | Local closed-loop well-posedness and bootstrap-domain existence | Establish local existence and uniqueness of the ES-1--ES-82 vector field on the admissible open domain `D_open`, and select a compact bootstrap set `K_0` with the initial state in its interior. No forward invariance, actuator feasibility, or global boundedness is claimed here. | ES-1--ES-16, ES-22--ES-23, ES-41--ES-53, ES-80--ES-82 | Assumption 1 regularity and initial funnel feasibility; Assumption 2 bounded admissible privacy parameters | Local Lipschitz/continuation-to-exit statement, maximal local interval, and a compact `K_0` embedded in `D_open`. | PO-03; PO-08; PO-09; PO-13; PO-11; PO-16B | OPEN | Equation revision if the specified local vector field is not well posed on `D_open`; Blueprint reopen only if the retained equations cannot define a local flow. |
| PO-16B | Forward continuation and operating-region invariance | Starting from PO-16A, PO-07, PO-11, and PO-13, prove that the maximal local solution cannot exit through the admissible operating-region, funnel, or actuator boundary, so the solution continues in the declared domain and all bootstrap bounds can be extended along the trajectory. This is the global/forward closure obligation formerly conflated with PO-16. | ES-1--ES-16, ES-22--ES-40, ES-41--ES-53, ES-62--ES-103 | Assumptions 1--2; PO-07; PO-11; PO-13; PO-16A | A continuation theorem with explicit exit-set exclusion and persistence in `Delta`; no use of PO-12, PO-14, or PO-15. | PO-12; PO-14; PO-15; Theorems 1--4 | OPEN | Equation revision if boundedness cannot exclude an exit; Blueprint reopen only if an additional stabilizing module would be required. |

## Dependency order

```text
PO-16A -> PO-03 -> PO-02 -> PO-10
PO-01 -------------^          |
PO-06 -> PO-08/PO-09 ---------|
PO-16A -> PO-13 --------------v
PO-02,PO-03,PO-06,PO-08,PO-09,PO-10,PO-13 -> PO-07
PO-07,PO-08,PO-09,PO-13,PO-16A -> PO-11 -> PO-16B
PO-02,PO-07,PO-11,PO-16B -> PO-12 -> PO-14
PO-04 -> PO-05 ---------------------------> PO-15
PO-12,PO-14 ------------------------------> PO-15
```

The diagram is a proof order, not a controller signal path. `PO-16A` is local existence only; `PO-16B` is forward/global continuation. Theorem 1 uses PO-07--PO-11 and PO-16A/PO-16B; Theorem 2 uses PO-12; Theorem 3 uses PO-14; Theorem 4 uses PO-15. The former aggregate `PO-16` is retired as a dependency label and must not be used without the `A` or `B` suffix.
...
| PO-03 | PROVED SUBJECT TO PO-16A | 2026-08-07 (conditional) | `derivation_stage_1_0807.md`, PO-03 | YES |
...
| PO-08 | PROVED SUBJECT TO PO-16A | 2026-08-07 (conditional) | `derivation_stage_2_5_0807.md`, PO-08 | YES |
| PO-09 | PROVED SUBJECT TO PO-16A | 2026-08-07 (conditional) | `derivation_stage_2_5_0807.md`, PO-09 | YES |
